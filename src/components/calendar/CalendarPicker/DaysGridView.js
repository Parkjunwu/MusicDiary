...
export default class DaysGridView extends Component {
  constructor(props) {
    ...
    this.initMonthSettings = props => {
      ...
      const firstDayOfMonth = momentSeoulTZ({ year, month, day: 1 });
  ...
  componentDidUpdate(prevProps) {
    ...
    // 뭐하는 애지 얘는?
    const propDiffs = Utils.shallowDiff(this.props, prevProps, 'selectedStartDate');
    // if (propDiffs.length) {
    if (propDiffs) {
      // Recreate days
    ...
    //   const { selectedStartDate, month } = this.props;
    //   const { selectedStartDate: prevSelStart } = prevProps;
    ...
    //   const isSelectedDiff = !Utils.compareDates(selectedStartDate, prevSelStart, 'day');
    ...
    //     Utils.compareDates(month, firstDayOfMonth)
    //     ))
    //   {
    ...
    //           const isSelected = Utils.compareDates(selectedStartDate, day);
    //           const isPrevSelected = Utils.compareDates(prevSelStart, day);
...
//   dateParamsObject: PropTypes.object,
// };
