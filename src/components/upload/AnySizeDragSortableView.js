...
// https://github.com/mochixuan/react-native-drag-sort/blob/master/lib/AnySizeDragSortableView.js 보고 수정
...
export default class AnySizeDragSortableView extends React.PureComponent {
  ...
  startTouch = (item, index) => {
    const {keyExtractor, headerViewHeight, wrapperStyle} = this.props
    ...
    // paddingLeft, paddingHorizontal 도 넣던지
    const paddingLeft = wrapperStyle?.padding;
    ...
    this.setState({ 
      ...
      selectedPosition: {
        ...
        left: parseInt(curLayout.x+0.5+paddingLeft),
        top: initTop,
        initTop,
  ...
  scrollTo = (height, animated = true) => {
    ...
    this.scrollRef && this.scrollRef.scrollToPosition(0, height, animated); // keyboard-aware-scroll-view
  }
  ...
  render() {
    ...
    const {dataSource, keyExtractor, renderItem, movedWrapStyle, wrapperStyle, body, placeholderTextColor, textColor, setBody, setFileAddingPosition, fontFamily} = this.props
    return (
        ...
        <KeyboardAwareScrollView
          ...
          <View style={styles.container}>
            ...
            {/* 여기에 Input 을 추가하면 되지 않을라나?
              ...
              이래 하면 되지 않을라나? 근데 key 가... 저기로 가네. 그럼 key 말고 다른걸로 받아야 될텐데. keyToIndexMap 를 다른걸로 만들어야 할듯. key 말고 뭐 fileKey 이렇게 바꾸던지 해서. 그럼... 문제가 이미지 말고 Input 위에 올라왔을 때 어떻게 처리할것인가.
            */}
