...
const useEditDiaryAndBoardCheckAndNavigate = () => {

  const navigation = useNavigation();
  // const isUseCache = () => isLoggedInVar();

  const editDiaryAndBoardCheckAndNavigate = async() => {
    ...
    const isEditBoard = await AsyncStorage.getItem(TEMPORARY_EDIT_BOARD);

    // console.log("isLocalDBEditDiary : "+ isLocalDBEditDiary);
    // console.log("isEditDiary : "+ isEditDiary);
    // console.log("isEditBoard : "+ isEditBoard);

    if(isLocalDBEditDiary && isEditDiary && isEditBoard) {
      console.error("isLocalDBEditDiary 랑 isEditDiary isEditBoard 셋 중 두개 이상 있음. 로직 확인 필요");
    }
    ...
    if(isEditBoard && isUseCache) {
    // if(isUseCache) {
      const navigateParams = JSON.parse(isEditBoard);
      ...
      //   prevBody: ["ㅁㄴㅇ"],
      // };
      return Alert.alert("수정 중인 게시물이 있습니다. 불러오시겠습니까?","취소 시 임시 저장된 내용은 삭제됩니다.",[
        {
          text:"이동",
          // onPress:()=>navigation.navigate("EditBoardForTemporaryBoardData",{
          //   ...navigateParams,
          // }),
          onPress:()=>navigation.navigate("BoardTab",{
            screen: 'EditBoardForTemporaryBoardData',
  ...
  return editDiaryAndBoardCheckAndNavigate;
};

export default useEditDiaryAndBoardCheckAndNavigate;
